...
public class CoreAdvanced : MonoBehaviour {
	float TimeStart;
	...
	public List<MatchAdvanced> Matches = new List<MatchAdvanced>();
	...
	void Awake()
	{
		...
		{
			if (File.Exists(Application.persistentDataPath + "/Data/NetworkAdvanced" + i + ".txt"))
			{
				StreamReader reader = new StreamReader(Application.persistentDataPath + "/Data/NetworkAdvanced" + i + ".txt");
			...
			{
				Networks.Add(new NeuralNetwork(new int[] { 8, 16, 8, 2 }));
		...
		for (int i = 0; i < NetworkCount - 1; i++)
		{
			for (int j = i + 1; j < NetworkCount; j++)
			{
				...
				Matches.Add(field.GetComponent<MatchAdvanced>());
				Matches[Matches.Count - 1].Index = matchId;
				matchId++;
			}
		}
	...
	void FixedUpdate()
	{
		...
		for (int i = 0; i < NetworkCount; i++)
		{
			PlayersTextString += Networks[i].Name + ": " + Networks[i].prevFitness + " | " + Networks[i].GetFitness() + "\n";
		}
	...
	void ResetGame()
	{
		...
		for (int i = 0; i < NetworkCount; i++)
		{
			...
			Networks[i].prevFitness = Networks[i].GetFitness();
		}
	...
	void OnDisable()
	{
		...
		for (int i = 0; i < NetworkCount; i++)
		{
			StreamWriter writer = new StreamWriter(Application.persistentDataPath + "/Data/NetworkAdvanced" + i + ".txt", false);
